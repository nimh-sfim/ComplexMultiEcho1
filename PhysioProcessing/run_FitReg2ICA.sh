...
micahs_dir=/data/holnessmn/UpToDate_ComplexME/ComplexMultiEcho1/PhysioProcessing/
...
# subject loop
for subj in sub-{01..25}; do                    # iterating thru all subjects
  ...
  maskname="full_mask.$subj+orig.HEAD"          # individual subject mask
  runnum=( 01 02 03 01 01 01 01 01 01 )         # run numbers
  tasklabeling=("wnw_run-1" "wnw_run-2" "wnw_run-3" \
                   "movie_run-1" "movie_run-2" "movie_run-3" \
                   "breathing_run-1" "breathing_run-2" "breathing_run-3")       # task labeling
  echo ${rundir[@]}
  echo "source /home/handwerkerd/InitConda.sh; \\" >> $swarmname            # sourcing from Dan's conda environment & inserting into swarm file
  for ridx in {0..8}; do                    # iterating through all runs
     if [ -d ${rundir[$ridx]} ]; then
        echo "Setting up ${rundir[$ridx]} FitICA calls"
        for tedtype in kic_kundu kic_minimal c70_kundu c70_minimal; do          # running all tedana types
cat << EOF >> $swarmname                        # insert into the swarm file
  python /data/holnessmn/UpToDate_ComplexME/ComplexMultiEcho1/PhysioProcessing/FitReg2ICA.py \\
    --registry ${rundir[$ridx]}/tedana_v23_${tedtype}_r${runnum[$ridx]}/desc-tedana_registry.json \\
    --regressors ../../../../Regressors/${subj}_RegressorModels_${tasklabeling[$ridx]}.tsv \\
    --outdir ${subj}_Reg2ICA --outprefix ${subj}_${tasklabeling[$ridx]}_${tedtype} \\
