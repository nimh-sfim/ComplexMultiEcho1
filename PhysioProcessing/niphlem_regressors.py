...
--tsv /Users/holnessmn/Desktop/BIDS_conversions/${sub}_physios/${sub}_task-${task}_run-${run}_physio.tsv.gz \
--json /Users/holnessmn/Desktop/BIDS_conversions/${sub}_physios/Originals/${sub}_task-${task}_run-${run}_physio.json \
--n_vols 299 \
--motion_demean /Users/holnessmn/Desktop/BIDS_conversions/${sub}_physios/${task}/run${run}/motion_demean.1D \
--motion_deriv /Users/holnessmn/Desktop/BIDS_conversions/${sub}_physios/${task}/run${run}/motion_deriv.1D \
--WM /Users/holnessmn/Desktop/BIDS_conversions/${sub}_physios/${task}/run${run}/mean.ROI.FSWe.1D \
--CSF /Users/holnessmn/Desktop/BIDS_conversions/${sub}_physios/${task}/run${run}/mean.ROI.FSvent.1D \
--prefix /Users/holnessmn/Desktop/BIDS_conversions/${sub}_physios/${task}/run${run}/${sub}_RegressorModels_${task}_run-${run}
...
parser.add_argument("--plots", dest="plots", help="Option to show plots")
...
# include as defaults in arg parser
# better names
#############
# module variables #
# ex: ECG = (2.0, 0.5), RESP = (0.5, 0.1)
#############
ecg_low = 2.0
ecg_high = 0.5
resp_low = 0.5
...
ecg_data = physio_data['Cardiac']
resp_data = physio_data['Respiratory']
...
# don't use "i" -> stands for index -> more informative name
...
if ARG.plots:
    ...
    def plot_WMnCSF(white_matter, csf_ventricles):
        ...
        plt.plot(frame_times, white_matter[:,0], label="White Matter")
        plt.plot(frame_times, 3 + csf_ventricles[:,0], label="Csf ventricles")
        plt.xlim([0,100])
        plt.xlabel("Time (s)", size=25)
        plt.legend(ncol=2, prop={'size':12}, loc="best")
        plt.tick_params(labelsize=20)
        plt.title("White Matter & CSF", fontsize=30)
        plt.yticks([])
        plt.show()
    ...
    def plot_variability(ecg_hrv,resp_rvt):
        ...
        plt.plot(frame_times, ecg_hrv[:,0], label="Heart Rate Variablity (HRV)")
        plt.plot(frame_times, resp_rvt[:,0], label="Respiration * Volume / Time (RVT)")
        plt.legend()
        plt.xlim([0,100])
        plt.xlabel("Time (s)", size=25)
