...
# My sample lines:
#   python3 /data/holnessmn/GLMs.py sub-01 ./ /data/NIMH_SFIM/handwerkerd/ComplexMultiEcho1/Data/sub-01/afniproc_orig/WNW/sub-01.results/ mylabel --noise_regressors all
...
def main():
    ...
    parser.add_argument("--noise_regressors", help="Type of noise regressor GLM to run",
        type=str)
    parser.add_argument(
...
def generate_GLM_statement(subj, GLMlabel, input_files, censorfile, regressors=None, include_motion=False, include_CSF=False):
    ...
    if (regressors):
        if regressors == "all":
            GLMstatement.append(
                getoutput(f"python3 /data/holnessmn/GLMs_Micah.py {subj} wnw --motion --wm --gm")
            )
        if regressors == "motion_wm":
            GLMstatement.append(
                getoutput(f"python3 /data/holnessmn/GLMs_Micah.py {subj} wnw --motion --wm")
            )
        if regressors == "motion_gm":
            GLMstatement.append(
                getoutput(f"python3 /data/holnessmn/GLMs_Micah.py {subj} wnw --motion --gm")
            )
        if regressors == "motion":
            GLMstatement.append(
                getoutput(f"python3 /data/holnessmn/GLMs_Micah.py {subj} wnw --motion")
            )
...
def generate_post_GLM_statements(subj, GLMlabel, censorfile):
    ...
    post_GLMstatements.extend([
        ...
        f"echo \"$blurs   # err_reml ACF blur estimates\" >> blur_est.{subj}.{GLMlabel}.1D",
        ""
    ])
